...
//SaleAnimalToken 실행 할 때 민팅 후 생성 된 주소 값을 넣어주어야 함
contract SaleAnimalToken{
    MintAnimalToken public mintAnimalTokenAddress; 
    
    constructor (address _mintAnimalTokenAddress){
        mintAnimalTokenAddress = MintAnimalToken(_mintAnimalTokenAddress);
    }           // mintAnimalToken을 실행할때 나온 주소값을 mintAnimalTokenAddress에 저장

    mapping(uint256 => uint256) public animalTokenPrices;  // animalTokenid 입력하면 가격 나옴

    uint256[] public onSaleAnimalTokenArray; //프론트엔드에서 판매중인 토큰을 확인 할 수 있게 해주는 배열

    //판매 등록 함수, 팔것에 id, 가격 필요
    function setForSaleAnimalToken(uint256 _animalTokenId,uint256 _price) public {
        address animalTokenOwner = mintAnimalTokenAddress.ownerOf(_animalTokenId); // 토큰 아이디를 입력하면 해당하는 주인에 대한 주소 값이 나옴
        ...
        require(mintAnimalTokenAddress.isApprovedForAll(animalTokenOwner,address(this)),"Animal Token owner did not approve token"); //address(this) : saleanimaltoken의 스마트 컨트랙트 주소, 이 주인이 이 컨트랙트에 판매 권한을 넣었는지 확인

        animalTokenPrices[_animalTokenId] = _price;

        onSaleAnimalTokenArray.push(_animalTokenId); // onSaleAnimalTokenArray 배열로 푸쉬해서 프론트엔드에서 판매중임을 확인할 수 있게 함
    }

    //구매 함수
    function purchaseAnimalToken(uint256 _animalTokenId) public payable{  //payable있어야 메틱(돈)이 왔다갔다 하는 함수 실행 가능
